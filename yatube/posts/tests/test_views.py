...
class PostViewsTests(TestCase):
    ...
    def test_templates_use_correct(self):
        for template, reverse in self.templates.items():
    ...
    def test_context_use_group(self):
        response = self.authorized_client.get(reverse('group_posts', kwargs={'slug': 'test'}))
        self.assertEqual(response.context.get('page')[0].text, 'Тестовый текст')
    ...
    def test_context_use_edit(self):
        response = self.authorized_client.get(reverse('post_edit', kwargs={'username': 'test_user', 'post_id': '1'}))
        form_fields = {       
            'text': forms.fields.CharField,
        }
        for value, expected in form_fields.items():
    ...
    def test_context_use_profile(self):
        response = self.authorized_client.get(reverse('profile', kwargs={'username': 'test_user'}))
        self.assertEqual(response.context.get('page')[0].text, 'Тестовый текст')
        self.assertEqual(response.context['author'].username, 'test_user')

    def test_context_use_post(self):
        response = self.authorized_client.get(reverse('post_view', kwargs={'username': 'test_user', 'post_id': '1'})) 
